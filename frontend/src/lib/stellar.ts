...
export async function simulateTransaction(
  ...
  try {
    ...
    // Simulate transaction with rate limiting
    const simulated = await rpcLimiter.execute(() =>
...
export async function buildAndSubmitTransaction(
  ...
  try {
    ...
    // Submit transaction
    const result = await rpcLimiter.execute(() =>
      sorobanServer.sendTransaction(signedTx as StellarSdk.Transaction)
    );

    // Wait for transaction to be included in a ledger
    let status = await rpcLimiter.execute(() =>
    ...
    while (status.status === 'NOT_FOUND' && attempts < maxAttempts) {
      ...
      status = await rpcLimiter.execute(() =>
        sorobanServer.getTransaction(result.hash)
      );
      attempts++;
    }
...
export async function callContract(
  ...
  try {
    ...
    // Use rate limiter to prevent exhausting RPC limits
    const result = await rpcLimiter.execute(() =>
      sorobanServer.simulateTransaction(transaction)
    );
